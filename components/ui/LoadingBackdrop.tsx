...
const LoadingBackdrop = ({ isFullScreen = false }: LoadingBackdropProps) => {
  return (
    <div
      role='status'
      className={`${isFullScreen ? 'fixed' : 'absolute'} hide-scrollbar inset-0 z-40 flex h-dvh w-full flex-col items-center justify-center bg-white/50 px-20 backdrop-blur-md`}
    >
      <JumpingDots />
    </div>
  );
